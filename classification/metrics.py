...
def get_pages():
    ...
    pages = []
    for page in cursor.execute(
            'select * from pages where pages.id not in(select page_id from page_changes)').fetchall():
...
def get_changes():
    ...
    for change in cursor.execute(
            'select * from page_changes where page_changes.raw_content <> \'\' order by created_at').fetchall():
...
if __name__ == '__main__':
    ...
    metrics = pd.DataFrame({a.__name__: a(G) for a in algorithms})

    metrics['namespace'] = metrics.index.map(lambda x: x.split(':')[0].strip())
    metrics['title'] = metrics.index.map(lambda x: x.split(':')[1].strip())

    print(metrics)

    print(metrics.groupby('namespace').mean().rank(axis=0, ascending=False))
